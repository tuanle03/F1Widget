                    ...
                    VStack(spacing: 3) {
                        ForEach(entry.standings.prefix(5)) { d in
                            HStack(spacing: 6) {
                                ...
                                Text(d.driver.permanentNumber ?? "—")
                                    .font(.system(size: 13, weight: .black, design: .rounded).monospacedDigit().italic())
                                    .foregroundStyle(ConstructorColors.color(forConstructorId: d.constructors.first?.constructorId ?? ""))
